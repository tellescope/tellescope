...
export const inclusionProjectionValidator = objectAnyFieldsValidator(numberValidatorBuilder({ lower: 1, upper: 1 }))
    ...
    (fields: any) => {
      ...
      for (const k in fields) {
        ...
        if (typeof val === 'string') {
          if (val.length > 2000) fields[k] = val.substring(0, 2000)
          continue
        } else if (typeof val === 'number' || val === null || typeof val === 'boolean') {
          continue // nothing to restrict on number type yet
        } else if (typeof val === 'object') {
          if (JSON.stringify(val).length > 25000) throw new Error(`object value for key ${k} exceeds the maximum length of 25000 characters in string representation`)
