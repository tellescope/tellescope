    ...
    return __awaiter(void 0, void 0, void 0, function () {
        ...
        return __generator(this, function (_e) {
            switch (_e.label) {
                ...
                case 8:
                    otherSdk_1 = new (_c.apply(Session, [void 0, (_d.authToken = (_e.sent()).authToken,
                            _d)]))();
                ...
                case 11:
                    ...
                    assert(apiKeys.some(function (k) { return k.id === otherKeyId; }), "admin list did not include another user's key ".concat(otherKeyId), "admin get_organization_api_keys returns keys created by other users in the org");
                    assert(apiKeys.some(function (k) { return k.id === adminKey_1.id; }), "admin list did not include the admin's own key", "admin get_organization_api_keys returns the admin's own keys");
                    assert(!!apiKeys.find(function (k) { return k.id === otherKeyId && k.creator === otherUser_1.id; }), "creator on the other user's key did not match", "returned key reports the correct creator");
                    // =============================================
                    // SECRET REDACTION (server-side, allowlist)
                    // =============================================
                    assert(apiKeys.every(function (k) { return k.key === undefined; }), "a returned record exposed a plaintext `key`", "no returned record exposes a plaintext key (legacy field)");
                    assert(apiKeys.every(function (k) { return k.hashedKey === undefined; }), "a returned record exposed `hashedKey`", "no returned record exposes hashedKey");
                    offending = apiKeys
                    ...
                    // =============================================
                    assert(apiKeys.every(function (k) { return k.businessId === sdk.userInfo.businessId; }), "a returned key belonged to a different business", "all returned keys are scoped to the admin's businessId");
                    // =============================================
                    // ADMIN GATING (negative tests)
                    // =============================================
                    return [4 /*yield*/, async_test("non-admin get_organization_api_keys is rejected (401)", function () { return otherSdk_1.api.api_keys.get_organization_api_keys(); }, { shouldError: true, onError: function (e) { return e.statusCode === 401 || /admin access only/i.test(e.message || ''); } })];
                case 12:
                    // =============================================
                    // ADMIN GATING (negative tests)
                    // =============================================
                    _e.sent();
                    return [4 /*yield*/, async_test("non-admin delete_organization_api_key is rejected (401)", function () { return otherSdk_1.api.api_keys.delete_organization_api_key({ id: adminKey_1.id }); }, { shouldError: true, onError: function (e) { return e.statusCode === 401 || /admin access only/i.test(e.message || ''); } })
                ...
                case 14:
                    afterGating = _e.sent();
                    assert(afterGating.apiKeys.some(function (k) { return k.id === adminKey_1.id; }), "rejected non-admin delete still removed the key", "rejected non-admin delete left the key intact");
                    // =============================================
                    // ADMIN DELETE another user's key
                    // =============================================
                    return [4 /*yield*/, async_test("admin delete_organization_api_key deletes another user's key", function () { return sdk.api.api_keys.delete_organization_api_key({ id: otherKeyId }); }, { shouldError: false, onResult: function () { return true; } })];
                case 15:
                    // =============================================
                    // ADMIN DELETE another user's key
                    // =============================================
                ...
                case 16:
                    afterDelete = _e.sent();
                    assert(!afterDelete.apiKeys.some(function (k) { return k.id === otherKeyId; }), "deleted key still present in org-wide list", "deleted key no longer appears in the org-wide list");
                    ...
                    return [4 /*yield*/, async_test("admin delete of a non-existent/cross-org key id is a safe no-op", function () { return sdk.api.api_keys.delete_organization_api_key({ id: '000000000000000000000000' }); }, { shouldError: false, onResult: function () { return true; } })];
                case 17:
                    // deleting a key in a different org (fabricated id) must be a no-op for our org
                    _e.sent();
                    return [4 /*yield*/, sdk.api.api_keys.get_organization_api_keys()];
                case 18:
                    afterNoop = _e.sent();
                    assert(afterNoop.apiKeys.some(function (k) { return k.id === adminKey_1.id; }), "no-op delete removed an unrelated key", "no-op cross-org delete left our org's keys intact");
                    return [3 /*break*/, 30];
                case 19:
    ...
    var runTests = function () { return __awaiter(void 0, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    _a.sent();
                    return [4 /*yield*/, organization_api_keys_tests({ sdk: sdk_1, sdkNonAdmin: sdkNonAdmin_1 })];
                case 2:
                    _a.sent();
                    return [2 /*return*/];
            }
