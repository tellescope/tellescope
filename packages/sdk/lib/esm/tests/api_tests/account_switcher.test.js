...
import { assert, async_test, handleAnyError, log_header, wait, } from "@tellescope/testing";
import { getBaseEmail } from "@tellescope/utilities";
        ...
        return __generator(this, function (_l) {
            switch (_l.label) {
                case 0:
                    ...
                    sdkOther = new Session({ host: host, apiKey: SDK_OTHER_API_KEY });
                    adminBaseEmail = getBaseEmail(sdk.userInfo.email);
                    _b = adminBaseEmail.split('@'), adminLocal = _b[0], adminDomain = _b[1];
                    naBaseEmail = getBaseEmail(sdkNonAdmin.userInfo.email);
                ...
                case 17:
                    ...
                    assert(linkedIds.includes(userPlusA.id), 'userPlusA missing from linked accounts', 'get_linked_accounts: includes plus sibling A');
                    assert(linkedIds.includes(userPlusB.id), 'userPlusB missing from linked accounts', 'get_linked_accounts: includes plus sibling B');
                    assert(linkedIds.includes(userPlusUppercase.id), 'uppercase plus variant missing from linked accounts', 'get_linked_accounts: case-insensitive base match');
                    assert(linkedIds.includes(userPlusInjection.id), 'regex-special plus variant missing from linked accounts', 'get_linked_accounts: regex special chars do not break matching');
                    assert(linkedIds.includes(userCrossOrg.id), 'cross-org sibling missing from linked accounts', 'get_linked_accounts: includes cross-org plus sibling');
                    assert(!linkedIds.includes(sdk.userInfo.id), 'caller present in linked accounts', 'get_linked_accounts: excludes caller');
                    assert(!linkedIds.includes(userOtherBase.id), 'unrelated-base user present in linked accounts', 'get_linked_accounts: excludes unrelated base email');
                    assert(!linkedIds.includes(userPrefixSwitcher.id), 'prefix-extended user present in linked accounts (anchored regex broken?)', 'get_linked_accounts: anchored regex excludes prefix-extended local part');
                    assert(!linkedIds.includes(userPlusLocked.id), 'locked user present in linked accounts', 'get_linked_accounts: excludes locked target');
                    ...
                    return [4 /*yield*/, async_test('get_linked_accounts callable by non-admin (noAccessPermissions)', function () { return sdkNonAdmin.api.users.get_linked_accounts(); }, { onResult: function (r) { return Array.isArray(r.linkedAccounts); } })
                        // ====================== B. switch_account ======================
                ...
                case 18:
                    // noAccessPermissions: non-admin can call without 403
                    _l.sent();
                    preSwitchTokenSession_1 = new Session({ host: host, authToken: sdk.authToken });
                    return [4 /*yield*/, async_test('switch_account: pre-switch source token is valid', function () { return preSwitchTokenSession_1.test_authenticated(); }, { expectedResult: 'Authenticated!' })];
                case 19:
                    _l.sent();
                    return [4 /*yield*/, sdk.api.users.switch_account({ targetUserId: userPlusB.id })];
                case 20:
                    switchResult = _l.sent();
                    assert(!!switchResult.authToken, 'no authToken returned from switch_account', 'switch_account: returns authToken');
                    assert(((_d = switchResult.user) === null || _d === void 0 ? void 0 : _d.id) === userPlusB.id, 'switch_account returned wrong user id', 'switch_account: returns target user record');
                    assert((((_e = switchResult.user) === null || _e === void 0 ? void 0 : _e.email) || '').toLowerCase() === emailPlusB.toLowerCase(), 'switch_account returned wrong email', 'switch_account: returns target email');
                    ...
                    assert(!((_f = switchResult.user) === null || _f === void 0 ? void 0 : _f.hashedPass), "switch_account response leaked hashedPass: ".concat(JSON.stringify(switchResult.user.hashedPass)), 'switch_account: response redacts hashedPass');
                    assert(!((_g = switchResult.user) === null || _g === void 0 ? void 0 : _g.hashedPassword), "switch_account response leaked hashedPassword: ".concat(JSON.stringify(switchResult.user.hashedPassword)), 'switch_account: response has no hashedPassword (defense-in-depth)');
                    switchedSession_1 = new Session({ host: host, authToken: switchResult.authToken });
                    return [4 /*yield*/, async_test('switch_account: new token authenticates', function () { return switchedSession_1.test_authenticated(); }, { expectedResult: 'Authenticated!' })];
                case 21:
                    _l.sent();
                    return [4 /*yield*/, switchedSession_1.api.users.getOne(userPlusB.id)];
                case 22:
                    meAfterSwitch = _l.sent();
                    assert((meAfterSwitch.email || '').toLowerCase() === emailPlusB.toLowerCase(), 'switched session does not resolve to target user', 'switch_account: new session authenticates as target user');
                ...
                case 23:
                    ...
                    assert(!newLinkedIds.includes(userPlusB.id), 'post-switch linked accounts include the new caller', 'post-switch get_linked_accounts: excludes new caller');
                    assert(newLinkedIds.includes(userPlusA.id), 'post-switch linked accounts missing other sibling', 'post-switch get_linked_accounts: includes other plus siblings');
                    assert(newLinkedIds.includes(sdk.userInfo.id), 'post-switch linked accounts missing original caller', 'post-switch get_linked_accounts: includes original caller');
                    // JWT invalidation (Step 13): the source token captured pre-switch must now be rejected.
                    return [4 /*yield*/, async_test('switch_account: source JWT invalidated after switch', function () { return preSwitchTokenSession_1.test_authenticated(); }, { shouldError: true, onError: function (e) { var _a; return /unauth|expired|invalid/i.test((e === null || e === void 0 ? void 0 : e.message) || ((_a = e === null || e === void 0 ? void 0 : e.toString) === null || _a === void 0 ? void 0 : _a.call(e)) || String(e)); } })
                ...
                case 26:
                    xorgSwitch = _l.sent();
                    assert(((_h = xorgSwitch.user) === null || _h === void 0 ? void 0 : _h.id) === userCrossOrg.id, 'cross-org switch returned wrong user', 'switch_account: cross-org switch returns target');
                    xorgSession_1 = new Session({ host: host, authToken: xorgSwitch.authToken });
                    return [4 /*yield*/, xorgSession_1.api.users.getOne(userCrossOrg.id)];
                case 27:
                    xorgMe = _l.sent();
                    assert((xorgMe.email || '').toLowerCase() === emailCrossOrg.toLowerCase(), 'cross-org session not scoped correctly', 'switch_account: cross-org session resolves to target user');
                    // Multi-tenant isolation: cross-org session must NOT see source-org users
                    return [4 /*yield*/, async_test('switch_account: cross-org session cannot see source-org user (tenant isolation)', function () { return xorgSession_1.api.users.getOne(sdk.userInfo.id); }, handleAnyError)
                ...
                case 30:
                    ...
                    downgradedSession_1 = new Session({ host: host, authToken: downgradeSwitch.authToken });
                    return [4 /*yield*/, async_test('switch_account: downgraded session cannot perform admin op (no privilege escalation)', function () { return downgradedSession_1.api.users.updateOne(userPlusA.id, { roles: ['Admin'] }, { replaceObjectFields: true }); }, handleAnyError)
                        // Re-auth sdk: previous switch invalidated its token (Step 13)
                    ];
                case 31:
                    _l.sent();
                ...
                case 33:
                    ...
                    _l.sent();
                    // 4. Cross-base attack: sdk (admin) → userOtherBase (different base) MUST be rejected.
                    return [4 /*yield*/, async_test('switch_account: rejects cross-base target', function () { return sdk.api.users.switch_account({ targetUserId: userOtherBase.id }); }, handleAnyError)];
                case 34:
                    // 4. Cross-base attack: sdk (admin) → userOtherBase (different base) MUST be rejected.
                    _l.sent();
                    return [4 /*yield*/, async_test('switch_account: source session unaffected after rejected cross-base attempt', function () { return sdk.test_authenticated(); }, { expectedResult: 'Authenticated!' })
                        // 5. Locked target: must reject
                    ];
                case 35:
                    _l.sent();
                    // 5. Locked target: must reject
                    return [4 /*yield*/, async_test('switch_account: rejects locked target', function () { return sdk.api.users.switch_account({ targetUserId: userPlusLocked.id }); }, handleAnyError)
                ...
                case 36:
                    ...
                    _l.sent();
                    // 6. Nonexistent target: must reject without leaking existence
                    return [4 /*yield*/, async_test('switch_account: rejects nonexistent target', function () { return sdk.api.users.switch_account({ targetUserId: '000000000000000000000000' }); }, handleAnyError)
                ...
                case 37:
                    ...
                    _l.sent();
                    // 7. Malformed targetUserId: validation error (does not reach handler / rate limit)
                    return [4 /*yield*/, async_test('switch_account: rejects malformed targetUserId', function () { return sdk.api.users.switch_account({ targetUserId: 'not-an-objectid' }); }, handleAnyError)
                ...
                case 38:
                    ...
                    _l.sent();
                    // 8. noAccessPermissions on switch: non-admin can switch to its own plus sibling
                    return [4 /*yield*/, async_test('switch_account: callable by non-admin (noAccessPermissions)', function () { return sdkNonAdmin.api.users.switch_account({ targetUserId: userNonAdminPlus.id }); }, { onResult: function (r) { var _a; return ((_a = r.user) === null || _a === void 0 ? void 0 : _a.id) === userNonAdminPlus.id && !!r.authToken; } })
                ...
                case 40:
                    // Re-auth sdkNonAdmin: previous switch invalidated its token (Step 13)
                    _l.sent();
                    noAuthSession_1 = new Session({ host: host });
                    return [4 /*yield*/, async_test('switch_account: rejects unauthenticated caller', function () { return noAuthSession_1.api.users.switch_account({ targetUserId: userPlusA.id }); }, handleAnyError)
                ...
                case 43:
                    ...
                    assert(!!switchLog, 'no account_switch audit log found for sdk → userPlusB', 'switch_account: HIPAA audit log written');
                    if (switchLog) {
                        assert(!!switchLog.info.sourceBusinessId && !!switchLog.info.targetBusinessId, 'audit log missing businessIds', 'switch_account: audit log captures both businessIds');
                        assert((switchLog.info.targetEmail || '').toLowerCase() === emailPlusB.toLowerCase(), 'audit log targetEmail mismatch', 'switch_account: audit log captures targetEmail');
                    }
                    return [4 /*yield*/, sdk.api.users.getOne(userUnverifTarget.id)];
                case 44:
                    beforeUnverif = _l.sent();
                    assert(beforeUnverif.verifiedEmail !== true, "test fixture userUnverifTarget was unexpectedly already verified", 'verifiedEmail propagation fixture: target starts unverified');
                ...
                case 47:
                    ...
                    assert(afterUnverif.verifiedEmail === true, "target verifiedEmail did not propagate from verified source", 'switch_account: verified source propagates verifiedEmail to target');
                ...
                case 53:
                    ...
                    return [3 /*break*/, 48];
                case 54:
                    assert(rateLimitTripped, "rate limit never tripped after 15 attempts (last error: ".concat((_k = lastError === null || lastError === void 0 ? void 0 : lastError.message) !== null && _k !== void 0 ? _k : 'none', ")"), 'switch_account: rate limit enforced');
