...
const inbox_threads_loading_tests = async () => {
  ...
  await sdk.api.inbox_threads.updateOne(phone.id, { phoneNumber: '+15555555555' })
  const phoneThread2 = await sdk.api.inbox_threads.createOne({
    ...defaultThreadFields, 
    title: 'Phone 2', 
    type: 'Phone', 
    threadId: '6',
  ...
  await async_test(
    'admin phoneNumber filter - SMS and Phone threads only',
    () => sdk.api.inbox_threads.load_threads({ phoneNumber: '+15555555555' } as any),
    { onResult: ({ threads }) => 
      threads.length === 2 
      && threads.some(t => t.type === 'SMS')
      && threads.some(t => t.type === 'Phone')
      && threads.every(t => t.type === 'SMS' || t.type === 'Phone')
    }
  )

  await async_test(
    'admin phoneNumber filter - different phone number',
    () => sdk.api.inbox_threads.load_threads({ phoneNumber: '+15555555556' } as any),
    { onResult: ({ threads }) => 
      threads.length === 1 
