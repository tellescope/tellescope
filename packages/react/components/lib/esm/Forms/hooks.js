...
import { MM_DD_YYYY_to_YYYY_MM_DD, TRANSLATION_LANGUAGES, append_current_utm_params, emit_gtm_event, field_can_autoadvance, getLocalTimezone, get_time_values, get_utm_params, languageLabelForCode, mm_dd_yyyy, object_is_empty, read_local_storage, replace_form_field_template_values, responses_satisfy_conditions, translationLanguageCodeForLanguage, update_local_storage } from "@tellescope/utilities";
...
export var language_code_for_legacy_form_language = function (value) {
    var _a, _b;
    if (!value)
        return 'en';
    if (value === 'Español' || value === 'Spanish')
        return 'es';
    if (TRANSLATION_LANGUAGES.some(function (l) { return l.code === value; }))
        return value;
    return (_b = (_a = TRANSLATION_LANGUAGES.find(function (l) { return l.label.toLowerCase() === value.trim().toLowerCase(); })) === null || _a === void 0 ? void 0 : _a.code) !== null && _b !== void 0 ? _b : 'en';
};
...
var browser_translation_language_code = function () {
    ...
    var code = translationLanguageCodeForLanguage({ iso6391: iso6391 }, { fallback: 'en' });
    return code === 'en' ? undefined : code;
};
var resolve_default_language_code = function (form, initialLanguage, detectBrowserLanguage) {
    var _a;
    if (initialLanguage)
        return language_code_for_legacy_form_language(initialLanguage);
    ...
    if (detected && ((_a = form === null || form === void 0 ? void 0 : form.translationConfigurations) === null || _a === void 0 ? void 0 : _a.some(function (t) { return t.language === detected; })))
        return detected;
    return language_code_for_legacy_form_language(form === null || form === void 0 ? void 0 : form.language);
};
...
export var useFormDisplayLanguage = function (_a) {
    ...
    var _b = useState(), selectedLanguageCode = _b[0], setLanguageCode = _b[1];
    var languageCode = useMemo(function () { return (selectedLanguageCode !== null && selectedLanguageCode !== void 0 ? selectedLanguageCode : resolve_default_language_code(form, initialLanguage, detectBrowserLanguage)); }, [selectedLanguageCode, form, initialLanguage, detectBrowserLanguage]);
    var _c = useState({}), maps = _c[0], setMaps = _c[1];
    var translationConfigurations = useMemo(function () { var _a; return (_a = form === null || form === void 0 ? void 0 : form.translationConfigurations) !== null && _a !== void 0 ? _a : []; }, [form === null || form === void 0 ? void 0 : form.translationConfigurations]);
    ...
    var formForDisplay = useMemo(function () {
        var _a, _b;
...
export var dateFromOffsetMs = function (offsetMs) {
    var d = new Date();
    d.setHours(0, 0, 0, 0);
...
export var useTellescopeForm = function (_a) {
    ...
    var _h = useFormResponses({ dontFetch: true }), _j = _h[1], updateFormResponse = _j.updateElement, updateLocalFormResponse = _j.updateLocalElement;
    var _k = useState(), customerId = _k[0], setCustomerId = _k[1];
    var _l = useState(root), activeField = _l[0], setActiveField = _l[1];
    var _m = useState(undefined), submittingStatus = _m[0], setSubmittingStatus = _m[1];
    var _o = useState(''), submitErrorMessage = _o[0], setSubmitErrorMessage = _o[1];
    var _p = useState(0), currentPageIndex = _p[0], setCurrentPageIndex = _p[1];
    var _q = useState([]), uploadingFiles = _q[0], setUploadingFiles = _q[1];
    var prevFieldStackRef = useRef([]);
    var lastNavigationDirectionRef = useRef(null);
    // Auto-advance state for form continuation
    var _t = useState(false), isAutoAdvancing = _t[0], setIsAutoAdvancing = _t[1];
    var autoAdvanceCompletedRef = useRef(false);
    var autoAdvanceStartTimeRef = useRef(null);
    var _u = useState({}), repeats = _u[0], setRepeats = _u[1];
    var gaEventRef = useRef({});
    var gtmEventRef = useRef({});
    var fieldViewCacheRef = useRef({}); // fieldId -> timestamp
    var goBackURL = '';
    try {
    ...
    var _v = useState(initializeFields()), responses = _v[0], setResponses = _v[1];
    useEffect(function () {
    ...
    var templatedResponses = useMemo(function () {
        ...
        return responses.map(function (response) {
            ...
            return __assign(__assign({}, response), { 
                ...
                fieldTitle: replace_form_field_template_values(originalField.title || '', { enduser: enduser, responses: responses }), fieldDescription: replace_form_field_template_values(originalField.description || '', { enduser: enduser, responses: responses }), 
                ...
                fieldHtmlDescription: replace_form_field_template_values(originalField.htmlDescription || '', { enduser: enduser, responses: responses, escapeNewlinesAsHTMLBreaks: true }), 
                // the nested field copy is what the live form renders — display-only, so translation is safe here
                field: __assign(__assign({}, response.field), { title: replace_form_field_template_values(translateDisplay(originalField.title || ''), { enduser: enduser, responses: responses }), description: replace_form_field_template_values(translateDisplay(originalField.description || ''), { enduser: enduser, responses: responses }), 
                    ...
                    htmlDescription: replace_form_field_template_values(translateDisplay(originalField.htmlDescription || ''), { enduser: enduser, responses: responses, escapeNewlinesAsHTMLBreaks: true, escapeHTMLValues: true }) }) });
        });
    ...
    var _w = useState(initializeFiles()), selectedFiles = _w[0], setSelectedFiles = _w[1];
    useEffect(function () {
        // Be very careful about refreshing data to avoid losing progress -- only in the case the selected form has changed
    ...
    var templatedActiveField = useMemo(function () {
    ...
    useEffect(function () {
        ...
        var nextNode = getNextField(activeField, currentResponse, responses, logicOptions);
        if (nextNode) {
    ...
    var validateBasicField = useCallback(function (field) {
        ...
        if (value.answer.type === 'dateString' && value.answer.value) {
            ...
            if (((_17 = field.options) === null || _17 === void 0 ? void 0 : _17.minDateOffsetMs) !== undefined) {
                var minDate = dateFromOffsetMs(field.options.minDateOffsetMs);
                var parsed = new Date(MM_DD_YYYY_to_YYYY_MM_DD(dateStr));
                parsed.setMinutes(parsed.getMinutes() + parsed.getTimezoneOffset());
                parsed.setHours(0, 0, 0, 0);
                if (parsed < minDate) {
                    return "Date must not be earlier than ".concat(mm_dd_yyyy(minDate));
                }
            }
            if (((_18 = field.options) === null || _18 === void 0 ? void 0 : _18.maxDateOffsetMs) !== undefined) {
                var maxDate = dateFromOffsetMs(field.options.maxDateOffsetMs);
                var parsed = new Date(MM_DD_YYYY_to_YYYY_MM_DD(dateStr));
                parsed.setMinutes(parsed.getMinutes() + parsed.getTimezoneOffset());
                parsed.setHours(0, 0, 0, 0);
                if (parsed > maxDate) {
                    return "Date must not be later than ".concat(mm_dd_yyyy(maxDate));
                }
            }
        }
        ...
        if (field.type === 'file' || field.type === 'files') {
            var blobCount = (_34 = (_33 = file.blobs) === null || _33 === void 0 ? void 0 : _33.length) !== null && _34 !== void 0 ? _34 : 0;
            var existingCount = 0;
        ...
        return __generator(this, function (_6) {
            switch (_6.label) {
                ...
                case 13: return [4 /*yield*/, _j.apply(_h, [__assign.apply(void 0, [__assign.apply(void 0, [(_l.accessCode = (_k), _l.responses = __spreadArray(__spreadArray([], responsesToSubmit_3, true), (existingResponses !== null && existingResponses !== void 0 ? existingResponses : []).filter(function (r) {
                                ...
                                , true), _l.automationStepId = automationStepId, _l.customerId = customerId, _l.productIds = responsesToSubmit_3.flatMap(function (r) { var _a, _b, _c; return (_c = (_b = (_a = r.field) === null || _a === void 0 ? void 0 : _a.options) === null || _b === void 0 ? void 0 : _b.productIds) !== null && _c !== void 0 ? _c : []; }), _l.utm = get_utm_params(), _l), (getEnduserAISummary && eId === enduserId ? { enduserAISummary: getEnduserAISummary() } : {})]), (viewedInLanguage && viewedInLanguage !== 'en' ? { viewedInLanguage: viewedInLanguage } : {})])])
