...
export const QuestionForField = ({
  ...
  const StediEligibility = customInputs?.['Stedi Eligibility'] ?? StediEligibilityInput
    ...
    <Flex column flex={1} style={{ marginBottom: spacing ?? 25 }} id={field.id}>
      {field.type !== 'Redirect' && field.title &&
        <Typography component="h4" style={{
          ...
          {form_display_text_for_language(form, field.title)}{!(field.isOptional || field.type === 'description' || field.type === 'Question Group' || field.type === 'Insurance' || field.type === 'Bridge Eligibility' || field.type === 'Candid Eligibility' || field.type === 'Stedi Eligibility') ? '*' : ''}
        </Typography>
      }
...

      {
        ...
        : field.type === 'Chargebee' ? (
          <Chargebee enduserId={enduserId} field={field} value={value.answer.value as any} onChange={onFieldChange as ChangeHandler<'Chargebee'>} setCustomerId={setCustomerId} form={form} responses={responses} />
        )
        ...
        : field.type === 'Stedi Eligibility' ? (
          <StediEligibility field={field} value={value.answer.value as any} form={form}
            enduser={enduser} responses={responses} enduserId={enduserId} formResponseId={formResponseId}
            onChange={onFieldChange as ChangeHandler<'Stedi Eligibility'>}
          />
        )
